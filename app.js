...
const DEFAULT_SPECIAL_TASKS = [
  ...
  { id:'sp2', name:'学而思语文', subject:'chinese', type:'chapters', weeklyTarget:3, needConfirm:true, chapters:[
    ...
    {id:'c15',name:'坐井观天',done:false},{id:'c16',name:'寒号鸟',done:false},{id:'c17',name:'我要的是葫芦',done:false},
    {id:'c18',name:'大禹治水',done:false},{id:'c19',name:'朱德的扁担',done:false},{id:'c20',name:'难忘的泼水节',done:false},
    {id:'c21',name:'古诗二首（夜宿山寺、敕勒歌）',done:false},{id:'c22',name:'雾在哪里',done:false},{id:'c23',name:'雪孩子',done:false},
    {id:'c24',name:'狐假虎威',done:false},{id:'c25',name:'狐狸分奶酪',done:false},{id:'c26',name:'纸船和风筝',done:false},{id:'c27',name:'风娃娃',done:false}
  ], icon:'📝' },
...
function initData() {
  ...
  // 配置迁移v4：更新默认任务配置（英语/数学/语文新任务，生活习惯留空）
  try {
    if (AppData.config && AppData.config.weeklyTasks) {
      // 检测是否是旧配置（包含生活习惯任务如"洗漱"，或v5配置如"贤贤老师"、"语文作业"）
      const allTasks = Object.values(AppData.config.weeklyTasks).flat();
      const hasOldHabit = allTasks.some(t => t.name === '洗漱' || t.name === '自己穿衣' || t.name === '整理学习物品');
      const hasOldEnglish = allTasks.some(t => t.name === '英语听读');
      const hasOldMath = allTasks.some(t => t.name === '数学作业' && !t.name.includes('贤贤') && !t.name.includes('西西'));
      const hasV5Config = allTasks.some(t => t.name === '贤贤老师的作业' || t.name === '语文作业');
      const hasOldXDF = allTasks.some(t => t.name === '新东方英语作业');
      if (hasOldHabit || hasOldEnglish || hasOldMath || hasV5Config || hasOldXDF) {
        console.log('[migration v6] 更新默认任务配置（检测到旧配置/v5配置）');
        AppData.config.weeklyTasks = JSON.parse(JSON.stringify(DEFAULT_TASKS));
      }
    }
  } catch (e) {
    console.error('[migration v4] 任务配置迁移失败:', e);
  }
  
  // 配置迁移v7：更新任务subject和category字段（总是执行，确保所有任务都有正确字段）
  try {
    const taskIdMap = {
      'cn_school': { subject: 'chinese', category: 'school' },
      'math_school': { subject: 'math', category: 'school' },
      'eng_school': { subject: 'english', category: 'school' },
      'cn_practice_reading': { subject: 'chinese', category: 'extracurricular' },
      'cn_reading': { subject: 'chinese', category: 'extracurricular' },
      'cn_practice': { subject: 'chinese', category: 'extracurricular' },
      'math_xes_1': { subject: 'math', category: 'extracurricular' },
      'math_xes_2': { subject: 'math', category: 'extracurricular' },
      'math_xes_3': { subject: 'math', category: 'extracurricular' },
      'math_xes_4': { subject: 'math', category: 'extracurricular' },
      'eng_xdf_1': { subject: 'english', category: 'extracurricular' },
      'eng_xdf_2': { subject: 'english', category: 'extracurricular' },
      'eng_xdf_3': { subject: 'english', category: 'extracurricular' },
      'eng_meibo': { subject: 'english', category: 'special' },
      'eng_meibo_class': { subject: 'english', category: 'special' },
      'eng_xdf_class': { subject: 'english', category: 'special' },
      'eng_listening': { subject: 'english', category: 'special' },
      'eng_animation': { subject: 'english', category: 'special' },
      'math_xes_class': { subject: 'math', category: 'special' },
      'sport_waterpolo': { subject: 'sport', category: 'school' },
      'sport_jump': { subject: 'sport', category: 'school' },
      'sport_swim': { subject: 'sport', category: 'school' },
      'sport_stretch': { subject: 'sport', category: 'school' },
      'sport_jump_swim': { subject: 'sport', category: 'school' }
    };
    ...
    for (let day = 1; day <= 7; day++) {
      ...
      if (tasks && Array.isArray(tasks)) {
        tasks.forEach(t => {
          if (taskIdMap[t.id]) {
            if (!t.subject) t.subject = taskIdMap[t.id].subject;
            if (!t.category) t.category = taskIdMap[t.id].category;
            migratedCount++;
          }
        });
      }
    ...
    if (migratedCount > 0) {
      console.log('[migration v7] 已更新 ' + migratedCount + ' 个任务的subject和category字段');
    } else {
      console.log('[migration v7] 所有任务已有正确的subject和category字段');
    }
    // 调试：打印今天的任务列表
    const today = getDayOfWeek();
    const todayTasks = AppData.config.weeklyTasks[today] || [];
    console.log('[migration v7] 今天(' + today + ')的任务数: ' + todayTasks.length);
    todayTasks.forEach(t => {
      console.log('[migration v7]   - ' + t.name + ' (id:' + t.id + ', type:' + t.type + ', subject:' + t.subject + ', category:' + t.category + ')');
    });
  } catch(e) {
    console.error('[migration v7] 任务字段迁移失败:', e);
  }
  
  // 配置迁移v8：强制更新任务配置为最新版本（保留已完成进度）
  try {
    const today = getDayOfWeek();
    const currentTasks = AppData.config.weeklyTasks[today] || [];
    const defaultTasks = DEFAULT_TASKS[today] || [];
    
    let needUpdate = false;
    if (currentTasks.length !== defaultTasks.length) {
      needUpdate = true;
    } else {
      for (let i = 0; i < currentTasks.length; i++) {
        if (currentTasks[i].id !== defaultTasks[i].id || currentTasks[i].name !== defaultTasks[i].name) {
          needUpdate = true;
          break;
        }
      }
    ...
    if (needUpdate) {
      console.log('[migration v8] 检测到任务配置不一致，强制更新为最新版本');
      AppData.config.weeklyTasks = JSON.parse(JSON.stringify(DEFAULT_TASKS));
      console.log('[migration v8] 任务配置已更新，今天任务数: ' + (AppData.config.weeklyTasks[today] || []).length);
    } else {
      console.log('[migration v8] 任务配置已是最新版本');
    }
  } catch(e) {
    console.error('[migration v8] 任务配置强制更新失败:', e);
  }
  
  // 配置迁移v9：更新奖励配置（添加娱乐时间兑换等新奖励）
  try {
    if (AppData.config && AppData.config.rewards) {
      const currentRewardIds = new Set(AppData.config.rewards.map(r => r.id));
      let needUpdate = false;
      
      // 检查是否缺少新奖励
      DEFAULT_REWARDS.forEach(defaultReward => {
        if (!currentRewardIds.has(defaultReward.id)) {
          needUpdate = true;
        }
      });
      
      if (needUpdate) {
        console.log('[migration v9] 检测到奖励配置不一致，更新为最新版本');
        // 保留已有的奖励配置，添加缺失的新奖励
        const existingRewards = AppData.config.rewards.filter(r => 
          DEFAULT_REWARDS.some(dr => dr.id === r.id)
        );
        const missingRewards = DEFAULT_REWARDS.filter(dr => 
          !existingRewards.some(er => er.id === dr.id)
        );
        AppData.config.rewards = [...existingRewards, ...missingRewards];
        console.log('[migration v9] 奖励配置已更新，当前奖励数: ' + AppData.config.rewards.length);
      } else {
        console.log('[migration v9] 奖励配置已是最新版本，奖励数: ' + AppData.config.rewards.length);
      }
    }
  } catch(e) {
  ...
  try {
    if (AppData.config && AppData.config.specialTasks) {
      const xesChinese = AppData.config.specialTasks.find(t => t.id === 'sp2' || t.name === '学而思语文');
      if (xesChinese) {
        const needUpdate = !xesChinese.chapters || 
                          xesChinese.chapters.length !== 28 || 
                          (xesChinese.chapters[0] && xesChinese.chapters[0].name !== '小蝌蚪找妈妈');
        if (needUpdate) {
          console.log('[migration] 更新学而思语文章节列表');
          const doneNames = new Set();
          if (xesChinese.chapters) {
            xesChinese.chapters.forEach(c => { if (c.done) doneNames.add(c.name); });
          }
          const defaultXes = DEFAULT_SPECIAL_TASKS.find(t => t.id === 'sp2');
          if (defaultXes) {
            xesChinese.chapters = defaultXes.chapters.map(c => ({
              id: c.id,
              name: c.name,
              done: doneNames.has(c.name)
            }));
            xesChinese.needConfirm = true;
          }
        }
      ...
      // 更新"课外绘本"为"课外阅读"
      const extraReading = AppData.config.specialTasks.find(t => t.id === 'sp1' || t.name === '课外绘本');
      if (extraReading && extraReading.name === '课外绘本') {
        extraReading.name = '课外阅读';
      }
    }
    
    // 初始化confirmHistory
    if (!AppData.confirmHistory) AppData.confirmHistory = [];
    
  } catch (e) {
    console.error('[migration] 配置迁移失败:', e);
  }
}
...
function showLevelUp(info) {
  const levelUpCharEl = document.getElementById('levelUpChar');
  if (levelUpCharEl) {
    const charImgPath = 'assets/characters/variants/' + (info.charImg || '09_Q版可爱_头像.png');
    levelUpCharEl.innerHTML = '<img src="' + charImgPath + '" style="width:80px;height:80px;border-radius:50%;object-fit:cover;border:3px solid #FFD700;box-shadow:0 0 20px rgba(255,215,0,0.6);" onerror="this.outerHTML=\'' + info.levelIcon + '\'">';
  }
...
function getTodayTasks() {
  ...
  try {
    const englishDailySpecials = [
      { id: 'eng_listening', name: '开车途中英语听力', icon: '🚗', subject: 'english', type: 'daily', category: 'special', cultivation: 20, star: 0, required: true },
      { id: 'eng_animation', name: '英语动画', icon: '📺', subject: 'english', type: 'daily', category: 'special', cultivation: 20, star: 0, required: true }
    ];
...
// 检查联动任务是否已在本周完成（如新东方作业3在周五或周六完成）
function isLinkedTaskCompleted(taskId) {
  if (!AppData._linkedTasks) AppData._linkedTasks = {};
  const weekKey = getWeekKey();
  return !!(AppData._linkedTasks[taskId] && AppData._linkedTasks[taskId][weekKey]);
}

// 标记联动任务本周已完成
function markLinkedTaskCompleted(taskId) {
  if (!AppData._linkedTasks) AppData._linkedTasks = {};
  const weekKey = getWeekKey();
  if (!AppData._linkedTasks[taskId]) AppData._linkedTasks[taskId] = {};
  AppData._linkedTasks[taskId][weekKey] = true;
}

function getTaskStatus(taskId) {
  // 联动任务特殊处理：如果本周已在另一天完成，则显示为已完成
  if (taskId === 'eng_xdf_3' && isLinkedTaskCompleted('eng_xdf_3')) {
    return 'done';
  }
...
function checkTask(taskId) {
  // 联动任务检查：如果本周已在另一天完成，提示已完成
  if (taskId === 'eng_xdf_3' && isLinkedTaskCompleted('eng_xdf_3')) {
    showToast('新东方作业3本周已完成，无需重复提交', 'info');
    return;
  }
  
  // 学而思语文任务特殊处理
  if (taskId && taskId.startsWith('xes_')) {
    const chapterId = taskId.replace('xes_', '');
    const xesTask = AppData.config.specialTasks.find(t => t.id === 'sp2' || t.name === '学而思语文');
    if (xesTask && xesTask.chapters) {
      const chap = xesTask.chapters.find(c => c.id === chapterId);
      if (chap && !chap.done) {
        // 检查是否已经在待确认中
        const existing = AppData.pendingReview.find(r => r.taskId === taskId);
        if (existing) {
        ...
        checkBadges();
        saveData();
        showToast('已提交，等待家长确认：学而思语文 - ' + chap.name, 'info');
        renderAll();
        return;
      }
    }
...
function confirmTask(reviewId) {
  ...
  // 学而思语文任务确认：标记章节完成
  if (review.taskId && review.taskId.startsWith('xes_')) {
    const chapterId = review.taskId.replace('xes_', '');
    const xesTask = AppData.config.specialTasks.find(t => t.id === 'sp2' || t.name === '学而思语文');
    if (xesTask && xesTask.chapters) {
      const chap = xesTask.chapters.find(c => c.id === chapterId);
      if (chap) chap.done = true;
      // 更新累计完成数
      if (!AppData.status.specialTotal) AppData.status.specialTotal = {};
      AppData.status.specialTotal['sp2'] = xesTask.chapters.filter(c => c.done).length;
    }
  } else {
    setTaskStatus(review.taskId, 'done');
    // 联动任务：标记本周已完成（只给一次积分）
    if (review.taskId === 'eng_xdf_3') {
      markLinkedTaskCompleted('eng_xdf_3');
    }
  }
  ...
  // 记录到确认历史
  if (!AppData.confirmHistory) AppData.confirmHistory = [];
  AppData.confirmHistory.push({
    id: review.id,
    taskId: review.taskId,
    taskName: review.taskName,
    taskType: review.taskType,
    cultivation: review.cultivation,
    date: review.date,
    time: review.time,
    status: 'confirmed',
    confirmDate: new Date().toLocaleDateString('zh-CN'),
    confirmTime: new Date().toLocaleTimeString('zh-CN', {hour:'2-digit',minute:'2-digit'})
  });
...
// 学生撤销待确认项目
function cancelPendingReview(reviewId) {
  const idx = AppData.pendingReview.findIndex(r => r.id === reviewId);
  if (idx === -1) return;
  const review = AppData.pendingReview[idx];
  setTaskStatus(review.taskId, 'pending');
...
function checkAllComplete() {
  ...
  if (allDone && !record.allComplete) {
    ...
    saveData();
    showToast('🎉 今日全部完成！获得20修为+2星', 'success');
  }
}
...
function getTodayCompletedCount() {
  ...
  tasks.forEach(t => {
    ...
    if (status === 'done' || status === 'pendingReview') count++;
  });
  return count;
}
...
function renderKanban() {
  ...
  // 今日任务
  let todayTasks = getTodayTasks().slice();
  if (filter !== 'all') todayTasks = todayTasks.filter(t => t.type === filter || t.subject === filter);
  ...
  // 已过期任务（生活习惯、运动、每日固定任务不计入已过期，因为无法后续弥补或每天会重新生成）
  let overdue = AppData.overdueTasks.filter(t => !t.resolved && t.type !== 'habit' && t.type !== 'sport' && t.type !== 'daily');
...
function renderReviewCard(review) {
  ...
  html += '<span style="font-size:12px;color:var(--gold-dark);font-weight:bold;margin-right:8px;">⏳ 待家长确认</span>';
  html += '<button class="btn btn-secondary btn-sm" onclick="cancelPendingReview(\'' + review.id + '\')">↩ 撤销</button>';
  html += '</div></div>';
  return html;
}
...
function dailySettlement() {
  ...
  const tasks = AppData.config.weeklyTasks[day] || [];
  tasks.forEach(t => {
    // 每日固定任务（type='daily'，包括语文、英语、数学、运动的今日任务）过期未完成时，不计入已过期
    // 因为这些任务每天都会重新生成，无需在已过期列表中重复显示
    if (t.required && !t.marker && record.tasks[t.id] !== 'done' && t.type !== 'daily') {
...
// ===== 学科页面渲染（三部分：校内作业/课外作业/专项任务） =====
function renderSubject(subject) {
  const subjectNames = { chinese:'语文', english:'英语', math:'数学' };
  const titleIds = { chinese:'chineseTodayTitle', english:'englishTodayTitle', math:'mathTodayTitle' };
  let allTasks = getTodayTasks().filter(t => t.type === subject || t.subject === subject);
  
  // 动态修改标题：周末显示"周末XX作业"，平时显示"今日XX作业"
  const day = new Date().getDay();
  const isWeekend = (day === 0 || day === 6);
  const titleEl = document.getElementById(titleIds[subject]);
  if (titleEl) titleEl.textContent = isWeekend ? '周末' + subjectNames[subject] + '作业' : '今日' + subjectNames[subject] + '作业';
  
  // 学而思语文任务已在getTodayTasks()中统一动态添加，此处不再重复添加
  
  // 按category分组
  const schoolTasks = allTasks.filter(t => t.category === 'school');
  const extraTasks = allTasks.filter(t => t.category === 'extracurricular');
  let specialTasks = allTasks.filter(t => t.category === 'special');
  
  // 强制确保英语每日专项任务被包含（如果没有被动态添加到getTodayTasks中）
  if (subject === 'english') {
    const requiredEnglishSpecials = [
      { id: 'eng_listening', name: '开车途中英语听力', icon: '🚗', subject: 'english', type: 'daily', category: 'special', cultivation: 20, star: 0, required: true },
      { id: 'eng_animation', name: '英语动画', icon: '📺', subject: 'english', type: 'daily', category: 'special', cultivation: 20, star: 0, required: true }
    ];
    requiredEnglishSpecials.forEach(reqTask => {
      const exists = specialTasks.some(t => t.id === reqTask.id);
      if (!exists) {
        console.log('[renderSubject] 强制添加英语专项任务:', reqTask.id, reqTask.name);
        specialTasks.push(Object.assign({}, reqTask));
      }
    });
    console.log('[renderSubject] 英语专项任务最终数量:', specialTasks.length);
    specialTasks.forEach((t, idx) => {
      console.log('[renderSubject]   专项任务' + idx + ':', t.id, t.name, 'category:', t.category, 'type:', t.type);
    });
  }
  
  // 晚训提示：周一、周四有晚训（水球），只完成校内作业
  let noticeHtml = '';
  if ((day === 1 || day === 4) && subject !== 'sport') {
    noticeHtml = '<div style="background:linear-gradient(135deg,#667eea,#764ba2);color:white;padding:12px 18px;border-radius:12px;margin-bottom:16px;font-size:15px;display:flex;align-items:center;gap:10px;box-shadow:0 4px 12px rgba(102,126,234,0.3);"><span style="font-size:22px;">🤽</span><span><strong>今天有晚训（水球）</strong>，只完成校内作业</span></div>';
  } else if (day === 3 && subject === 'english') {
    noticeHtml = '<div style="background:linear-gradient(135deg,#f093fb,#f5576c);color:white;padding:12px 18px;border-radius:12px;margin-bottom:16px;font-size:15px;display:flex;align-items:center;gap:10px;box-shadow:0 4px 12px rgba(240,147,251,0.3);"><span style="font-size:22px;">🎓</span><span><strong>今天只有美博英语课</strong></span></div>';
  }
  
  // 渲染任务列表的辅助函数（简化版，确保每个任务都有复选框）
  function renderTaskList(tasks, emptyText) {
    if (!tasks || tasks.length === 0) {
      return '<div style="text-align:center;color:#ccc;padding:20px 0;font-size:13px;">' + emptyText + '</div>';
    }
    let html = '';
    for (let i = 0; i < tasks.length; i++) {
      ...
      const checkClass = isDone ? 'checked' : (isPending ? 'pending' : '');
      const checkIcon = isDone ? '✓' : (isPending ? '⏳' : '');
      const disabledStyle = (isDone || isPending) ? ' style="opacity:0.6;pointer-events:none;"' : '';
      const classTag = (t.type === 'class') ? '<span style="background:linear-gradient(135deg,#ffd700,#ff8c00);color:white;padding:2px 8px;border-radius:6px;font-size:11px;margin-left:8px;font-weight:normal;">上课</span>' : '';
      const cultivation = t.cultivation || 0;
      
      // 生成完整的任务项HTML（确保有复选框）
      html += '<div class="task-item"' + disabledStyle + '>' +
        '<div class="task-check ' + checkClass + '" onclick="checkTask(\'' + t.id + '\')">' + checkIcon + '</div>' +
        '<div class="task-info">' +
          '<div class="task-name">' + t.name + classTag + '</div>' +
        '</div>' +
        '<div class="task-cultivation">+' + cultivation + '✨</div>' +
      '</div>';
    }
    return html;
  }
  ...
  // 第一部分：今日校内作业
  const schoolHtml = noticeHtml + renderTaskList(schoolTasks, '今天没有校内作业');
  document.getElementById(subject + 'TodayTasks').innerHTML = schoolHtml;
  
  // 第二部分：今日课外作业
  const extraHtml = renderTaskList(extraTasks, '今天没有课外作业');
  const extraContainer = document.getElementById(subject + 'ExtraTasks');
  if (extraContainer) extraContainer.innerHTML = extraHtml;
  
  // 第三部分：专项任务（上课类任务）
  console.log('[renderSubject] ' + subject + ' - 专项任务数量:', specialTasks.length);
  specialTasks.forEach((t, idx) => {
    console.log('[renderSubject]   专项任务' + idx + ':', t.id, t.name, 'category:', t.category, 'type:', t.type, 'subject:', t.subject);
  });
  const specialHtml = renderTaskList(specialTasks, '暂无专项任务');
  console.log('[renderSubject] ' + subject + ' - 专项任务HTML长度:', specialHtml.length);
  console.log('[renderSubject] ' + subject + ' - 专项任务HTML:', specialHtml.substring(0, 500));
  const specialContainer = document.getElementById(subject + 'SpecialTasks');
  if (specialContainer) {
    specialContainer.innerHTML = specialHtml;
  } else {
    console.error('[renderSubject] 未找到容器:', subject + 'SpecialTasks');
  }
}
...
function renderSport() {
  const allTasks = getTodayTasks().filter(t => t.type === 'sport' || t.subject === 'sport');
  
  // 今日运动
  let todayHtml = '';
  if (allTasks.length === 0) {
    todayHtml = '<div style="text-align:center;color:#ccc;padding:20px 0;font-size:13px;">今天没有运动任务</div>';
  } else {
    allTasks.forEach(t => {
      ...
      todayHtml += '<div class="task-info"><div class="task-name">' + t.name + '</div></div>';
      todayHtml += '<div class="task-cultivation">+' + t.cultivation + '✨</div>';
      todayHtml += '</div>';
    });
  }
  document.getElementById('sportTodayTasks').innerHTML = todayHtml;
  
  // 专项运动（留空）
  document.getElementById('sportSpecialTasks').innerHTML = '<div style="text-align:center;color:#ccc;padding:20px 0;font-size:13px;">暂无专项运动任务</div>';
}
// ===== 猫小九历险记模块 =====
function renderAdventure() {
  const info = getLevelInfo();
  // 顶部大图标：显示当前境界对应的猫小九形象图片
  const advCharImgEl = document.getElementById('advCharImg');
  if (advCharImgEl) {
    const currentLevel = LEVEL_SYSTEM[AppData.status.levelIndex] || LEVEL_SYSTEM[0];
    const charImgPath = 'assets/characters/variants/' + (currentLevel.charImg || '09_Q版可爱_头像.png');
    advCharImgEl.innerHTML = '<img src="' + charImgPath + '" alt="' + info.levelName + '" style="width:100%;height:100%;border-radius:50%;object-fit:cover;display:block;" onerror="this.parentElement.textContent=\'' + info.levelIcon + '\'">';
  }
...
function renderReward() {
  try {
  console.log('[奖励中心] 开始渲染');
  if (!AppData.config.rewards || !Array.isArray(AppData.config.rewards)) {
    AppData.config.rewards = JSON.parse(JSON.stringify(DEFAULT_REWARDS));
    console.log('[奖励中心] 奖励配置已重置为默认值');
  }
  console.log('[奖励中心] 奖励数量: ' + AppData.config.rewards.length);
  ...
  AppData.config.rewards.forEach(reward => {
    ...
    const canAfford = (AppData.status.stars || 0) >= reward.cost;
    ...
    html += '<div class="reward-cost" style="color:#DAA520;font-weight:bold;">⭐ ' + reward.cost + '</div>';
  ...
  document.getElementById('rewardRecords').innerHTML = recordHtml;
    ...
    console.error('[奖励中心] 渲染失败:', e);
    const grid = document.getElementById('rewardGrid');
    if (grid) grid.innerHTML = '<div style="padding:20px;color:#999;text-align:center;">奖励加载失败，请刷新页面</div>';
  }
}
...
function renderCharacterShop() {
  ...
  CHARACTER_IMAGES.forEach(c => {
    ...
    const canAfford = (AppData.status.stars || 0) >= c.cost;
    const imgPath = c.category === '猫小九' ? 'assets/characters/variants/' + c.file : 'assets/other_characters/' + c.file;
  ...
  container.innerHTML = html;
    ...
    console.error('[形象兑换] 渲染失败:', e);
    const container = document.getElementById('characterShop');
    if (container) container.innerHTML = '<div style="padding:20px;color:#999;text-align:center;">形象加载失败</div>';
  }
}
...
function redeemCharacter(charId) {
  ...
  showModal('确认兑换', '确定花费 ' + char.cost + ' ⭐ 兑换【' + char.name + '】形象吗？兑换后永久解锁，可在角色图鉴中设为头像。', () => {
    AppData.status.stars = (AppData.status.stars || 0) - char.cost;
...
function renderBackgroundShop() {
  ...
  BACKGROUND_IMAGES.forEach(bg => {
    ...
    const canAfford = (AppData.status.stars || 0) >= bg.cost;
  ...
  container.innerHTML = html;
    ...
    console.error('[背景兑换] 渲染失败:', e);
    const container = document.getElementById('backgroundShop');
    if (container) container.innerHTML = '<div style="padding:20px;color:#999;text-align:center;">背景加载失败</div>';
  }
}
...
function redeemBackground(bgId) {
  ...
  showModal('确认兑换', '确定花费 ' + bg.cost + ' ⭐ 兑换【' + bg.name + '】背景吗？兑换后永久解锁，可在个性化设置中使用。', () => {
    AppData.status.stars = (AppData.status.stars || 0) - bg.cost;
    if (!AppData.unlockedBackgrounds) AppData.unlockedBackgrounds = [];
...
function redeemReward(rewardId) {
  ...
  showModal('确认兑换', 
    ...
    () => {
      ...
      // 娱乐时间兑换：增加entertainmentMinutes
      if (reward.isEntertainment) {
        AppData.status.entertainmentMinutes = (AppData.status.entertainmentMinutes || 0) + 15;
        if (AppData.status.entertainmentMinutes > 120) AppData.status.entertainmentMinutes = 120;
      }
      AppData.redemptionRecords.unshift({
        id: uid(),
        rewardId: reward.id,
...
function showParentTab(tab) {
  ...
  if (tab === 'confirm') renderConfirmManagement();
...
// 确认管理界面：显示完整历史记录，可按状态筛选
let confirmFilter = 'all';
function renderConfirmManagement() {
  const container = document.getElementById('confirmManagementList');
  if (!container) return;
  ...
  // 已确认/已驳回任务（从历史记录中收集）
  if (AppData.confirmHistory) {
    AppData.confirmHistory.forEach(r => {
      allRecords.push({
        id: r.id,
        type: 'task',
        name: r.taskName,
        ...
        time: r.time,
        status: r.status,
        statusText: r.status === 'confirmed' ? '已确认' : '已驳回',
        statusColor: r.status === 'confirmed' ? 'var(--green)' : 'var(--red)'
      });
    });
  }
  ...
  // 按状态筛选
  const filteredRecords = confirmFilter === 'all' ? allRecords : allRecords.filter(r => r.status === confirmFilter);
  
  // 统计
  const pendingCount = allRecords.filter(r => r.status === 'pending').length;
  const confirmedCount = allRecords.filter(r => r.status === 'confirmed').length;
  const rejectedCount = allRecords.filter(r => r.status === 'rejected').length;
  
  let html = '<h3 style="margin-bottom:12px;font-size:16px;">📋 确认管理</h3>';
  
  // 统计卡片
  html += '<div style="display:flex;gap:12px;margin-bottom:16px;flex-wrap:wrap;">';
  html += '<div style="flex:1;min-width:100px;background:#FFF8E1;padding:12px;border-radius:10px;text-align:center;">';
  html += '<div style="font-size:24px;font-weight:bold;color:var(--gold-dark);">' + pendingCount + '</div>';
  html += '<div style="font-size:12px;color:#666;">待确认</div></div>';
  html += '<div style="flex:1;min-width:100px;background:#E8F5E9;padding:12px;border-radius:10px;text-align:center;">';
  html += '<div style="font-size:24px;font-weight:bold;color:var(--green);">' + confirmedCount + '</div>';
  html += '<div style="font-size:12px;color:#666;">已确认</div></div>';
  html += '<div style="flex:1;min-width:100px;background:#FFEBEE;padding:12px;border-radius:10px;text-align:center;">';
  html += '<div style="font-size:24px;font-weight:bold;color:var(--red);">' + rejectedCount + '</div>';
  html += '<div style="font-size:12px;color:#666;">已驳回</div></div>';
  html += '</div>';
  ...
  filters.forEach(f => {
    const active = confirmFilter === f.key ? 'background:var(--primary);color:#fff;' : 'background:#f5f5f5;color:#666;';
    html += '<button class="btn btn-sm" style="' + active + '" onclick="setConfirmFilter(\'' + f.key + '\')">' + f.label + '（' + f.count + '）</button>';
  });
  html += '</div>';
  
  // 记录列表
  if (filteredRecords.length === 0) {
    html += '<div style="text-align:center;color:#ccc;padding:40px;">暂无记录</div>';
  } else {
    html += '<div style="max-height:500px;overflow-y:auto;">';
    filteredRecords.forEach(r => {
      const typeLabels = { chinese:'语文', english:'英语', math:'数学', sport:'运动', habit:'生活习惯', study:'学习' };
      html += '<div class="pending-review-item" style="opacity:' + (r.status === 'pending' ? '1' : '0.85') + ';">';
      html += '<div class="review-info"><div class="review-name">' + r.name + '</div>';
      html += '<div class="review-meta">' + (r.date || '') + ' ' + (r.time || '') + ' · ' + (typeLabels[r.taskType] || '') + (r.cultivation > 0 ? ' · +' + r.cultivation + '修为' : '') + '</div></div>';
      html += '<div class="review-actions">';
      html += '<span style="font-size:12px;font-weight:bold;color:' + r.statusColor + ';">' + r.statusText + '</span>';
      if (r.status === 'pending') {
        html += '<button class="btn btn-success btn-sm" onclick="confirmTask(\'' + r.id + '\')">✓ 确认</button>';
        html += '<button class="btn btn-danger btn-sm" onclick="rejectTask(\'' + r.id + '\')">✗ 驳回</button>';
      }
...
// 设置确认管理筛选条件
function setConfirmFilter(filter) {
  confirmFilter = filter;
  renderConfirmManagement();
}
...
// 重置修为和星星（保留配置）
function resetCultivationAndStars() {
  showModal('🔄 重置修为和星星', '<p>确定要将修为和星星归零吗？</p><p>将重置：总修为、当前境界、星星、连续打卡天数、升级记录、每日打卡记录</p><p>将保留：任务配置、奖励配置、徽章配置、专项任务进度、个性化设置</p><p style="color:var(--gold-dark);">此操作不可撤销</p>', () => {
    // 重置状态
    AppData.status.totalCultivation = 0;
    AppData.status.levelIndex = 0;
    AppData.status.starIndex = 0;
    AppData.status.stars = 0;
    AppData.status.coins = 0;
    AppData.status.entertainmentMinutes = 0;
    AppData.status.continuousDays = 0;
    AppData.status.lastCompleteDate = null;
    AppData.status.allCompleteCount = 0;
    AppData.status.levelUpCount = 0;
    AppData.status.redeemCount = 0;
    AppData.status.jumpRopePassCount = 0;
    AppData.status.jumpBest = 0;
    AppData.status.footballCount = 0;
    
    // 清空每日记录和升级记录
    AppData.dailyRecords = {};
    AppData.levelUpRecords = [];
    AppData.pendingReview = [];
    AppData.confirmHistory = [];
    
    // 重置徽章获得状态（保留徽章配置）
    if (AppData.config.badges) {
      AppData.config.badges.forEach(b => { b.earned = false; });
    }
    
    saveData();
    showToast('修为和星星已归零，配置已保留', 'success');
    renderAll();
  });
}
